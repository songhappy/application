...
class TextClassifier(param: AbstractTextClassificationParams) extends Serializable {
  ...
  /**
    ...
    */
  def train(modelPath: String): Unit = {
    ...
    // For large dataset, you might want to get such RDD[(String, Float)] from HDFS
    val dataRdd = sc.parallelize(loadRawData(), param.partitionNum).sample(true, 0.2)
    val (word2Meta, word2Vec) = analyzeTexts(dataRdd)
    val word2MetaBC = sc.broadcast(word2Meta)
    val word2VecBC = sc.broadcast(word2Vec)
    ...
    val x = vectorizedRdd.count()
    val y = vectorizedRdd.take(1).toList
    println("data size" + x)
    println("feature size" + y(0)._1.size)
    ...
    val optmodel: Module[Float] = optimizer
...
class TextClassificationParams(override val baseDir: String = "./",
